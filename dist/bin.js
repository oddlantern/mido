...
async function main() {
	...
	if (command === "affected") {
		...
		const { runAffected } = await import("./affected-QD7DHumd.js");
	...
	if (command === "add") {
		const { runAdd } = await import("./add-IELEe3sB.js");
		const exitCode = await runAdd();
		process.exit(exitCode);
	}
	if (command === "init") {
		const { runInit } = await import("./init-gYVhJ9ME.js");
	...
	if (command === "graph") {
		...
		const { runGraph } = await import("./graph-CMID5s6Y.js");
	...
	if (command === "doctor") {
		const { runDoctor } = await import("./doctor-DkoHDX3k.js");
		const exitCode = await runDoctor(parsers);
		process.exit(exitCode);
	}
	if (command === "dev") {
		const verbose = args.includes("--verbose");
		const { runDev } = await import("./dev-D4AqoTFo.js");
	...
	if (command === "lint") {
		...
		const { runLint } = await import("./lint-BWsrXs2i.js");
	...
	if (command === "fmt") {
		...
		const pkg = getFlagValue(args, "--package");
		const ecosystem = getFlagValue(args, "--ecosystem");
		const { runFmt } = await import("./fmt-CqX-rkaR.js");
	...
	if (command === "generate") {
		...
		const dryRun = args.includes("--dry-run");
		const { runGenerate } = await import("./generate-MfgNv-k2.js");
	...
	if (command === "outdated") {
		...
		const { runOutdated } = await import("./outdated-C51ipVn4.js");
	...
	if (command === "upgrade") {
		...
		const verify = args.includes("--verify");
		const dryRun = args.includes("--dry-run");
		const { runUpgrade } = await import("./upgrade-D1gMZDOM.js");
	...
	if (command === "test") {
		...
		const pkg = getFlagValue(args, "--package");
		const ecosystem = getFlagValue(args, "--ecosystem");
		const { runTest } = await import("./test-DdNSZer2.js");
	...
	if (command === "build") {
		...
		const all = args.includes("--all");
		const pkg = getFlagValue(args, "--package");
		const { runBuild } = await import("./build-B--dKCZF.js");
	...
	if (command === "ci") {
		const verbose = args.includes("--verbose");
		const { runCi } = await import("./ci-CzatmbMR.js");
		const exitCode = await runCi(parsers, { verbose });
		process.exit(exitCode);
	}
	if (command === "pre-commit") {
		const { runPreCommit } = await import("./pre-commit-BFJ3sPzu.js");
	...
	if (command === "commit-msg") {
		...
		const { runCommitMsg } = await import("./commit-msg-lSTbey6Q.js");
	...
	if (command === "why") {
		...
		const json = args.includes("--json");
		const { runWhy } = await import("./why-CkhTbniB.js");
	...
	if (command === "rename") {
		...
		const dryRun = args.includes("--dry-run");
		const { runRename } = await import("./rename-vdZz_4q6.js");
