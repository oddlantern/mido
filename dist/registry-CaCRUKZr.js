...
async function executeTypescriptAssetGeneration(_pkg, root, context) {
	...
	for (const category of manifest.categories) {
		...
		hasPathContent = true;
		const constName = `${toPascalCase$2(category.name)}Paths`;
		...
		pathLines.push("");
		pathLines.push(`export type ${toPascalCase$2(category.name)}Key = keyof typeof ${constName};`);
		pathLines.push("");
	}
	...
	if (svgEntries.length > 0 && sourceDir) {
		...
		for (const category of manifest.categories) {
			...
			const constName = `${toPascalCase$2(category.name)}Svg`;
			const entries = [];
...
function generateCategoryClass(category, prefix, packageName) {
	...
	const className = `${prefix}${toPascalCase$1(category.name)}Icon`;
	const lines = [];
...
function generateThemeVariantClass(variant, prefix, packageName) {
	const className = `${prefix}${toPascalCase$1(variant.category)}Icon`;
	const lines = [];
	lines.push(`abstract final class ${className} {`);
...
//# sourceMappingURL=registry-CaCRUKZr.js.map