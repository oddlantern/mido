...
import { n as loadPlugins, t as PluginRegistry } from "./registry-1jUR3GeP.js";
...
async function runEcosystemCommand(parsers, options, runnerConfig) {
	...
	for (const [ecosystem, packages] of grouped) {
		...
		for (const { pkg, result } of results) {
			if (!result.success) {
				...
				diag.error(`${runnerConfig.action} failed: ${pkg.path}`, { detail: firstLine });
			}
	...
	if (!quiet || diag.hasIssues) console.log(formatDiagnostics(diag));
	return diag.hasErrors ? 1 : 0;
}
//#endregion
export { runEcosystemCommand as t };

//# sourceMappingURL=ecosystem-runner-CUqggDrf.js.map