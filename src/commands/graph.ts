...
interface GraphNode {
  ...
  readonly type: "package" | "generated";
}

interface GraphEdge {
  readonly source: string;
  readonly target: string;
  readonly type: "dependency" | "bridge" | "generated";
...
function buildGraphData(
  ...
  for (const [path, pkg] of packages) {
    nodes.push({ id: path, ecosystem: pkg.ecosystem, name: pkg.name, type: "package" });
    nodeIds.add(path);

    // Add dependency edges (local workspace deps)
    for (const dep of pkg.localDependencies) {
      edges.push({ source: dep, target: path, type: "dependency" });
    }
  }

  // Add bridge edges and generated nodes
...
function generateAscii(
  ...
  for (const edge of edges) {
    ...
    if (existing) {
      ...
      outgoing.set(edge.source, [label]);
    }
  }
